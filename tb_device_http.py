...
class TBHTTPDevice:
    ...
    def __init__(self, host: str, token: str, name: str = None, chunk_size: int = 0):
        ...
        self.__config = {"host": host, "token": token, "name": name, "timeout": 30}
        self.__worker = {
            "publish": {
                "queue": queue.Queue(),
            ...
            "attributes": {
                "thread": threading.Thread(
                    target=self.__subscription_worker,
                    daemon=True,
    ...
    def __get_firmware(self, fw_info):
        chunk_count = (
            ceil(fw_info.get(FW_SIZE_ATTR, 0) / self.chunk_size)
        ...
        for chunk_number in range(chunk_count + 1):
            params = {
                ...
                if self.chunk_size < fw_info.get(FW_SIZE_ATTR, 0)
    ...
    def send_telemetry(
        self, telemetry: dict, timestamp: datetime = None, queued: bool = True
    ):
    ...
    def request_attributes(
        self, client_keys: list = None, shared_keys: list = None
    ) -> dict:
    ...
    def __subscription_worker(self, endpoint: str, timeout: int = None):
        ...
        params = {"timeout": (timeout or self.timeout) * 1000}
        url = {
        ...
        while not stop_event.is_set():
            response = self.__session.get(
                url=url[endpoint], params=params, timeout=params["timeout"]
            )
    ...
    @classmethod
    def provision(
        cls, host: str, device_name: str, device_key: str, device_secret: str
    ):
        ...
        if (
            ...
            and device["credentialsType"] == "ACCESS_TOKEN"
        ):
...
class TBHTTPClient(TBHTTPDevice):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.logger.critical(
            "TBHTTPClient class is deprecated, please use TBHTTPDevice"
        )
